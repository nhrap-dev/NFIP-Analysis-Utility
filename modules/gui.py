def run_gui():
    ...
    try:
        ...
        def browse_depths():
            ...
            text = str(len(depths_directory)) + ' depth files selected' if len(depths_directory) > 1 else str(len(depths_directory)) + ' depth file selected'
        ...
        def run():
            try:
                functions.AnalyzeNFIP(depths_directory, policy_directory, output_directory, export_shapefile=opt_shp.get())
            except:
        ...
        pathlabel_policy = tk.Label(root, bg='#282a36', fg='#f8f8f2', font='Helvetica 8')
        pathlabel_policy.grid(row=row, column=0, pady=(5, 0), padx=30, sticky='W')
        row += 1

        # Input depth directory
        label_dir_depths = tk.Label(root, text='Select all depth files:', font='Helvetica 10 bold', bg='#282a36', fg='#f8f8f2')
        ...
        pathlabel_depth = tk.Label(root, bg='#282a36', fg='#f8f8f2', font='Helvetica 8')
        pathlabel_depth.grid(row=row, column=0, pady=(5, 0), padx=30, sticky='W')
        row += 1

        # Output checkbox label
        label_outputMetadata = tk.Label(root, text='Check to export the spatial data:', font='Helvetica 10 bold', bg='#282a36', fg='#f8f8f2')
        label_outputMetadata.grid(row=row, column=0, padx=20, pady=(20, 0), sticky='W')
        row += 1
        ...
        browsebutton_dat = tk.Button(root, text="Browse", command=browse_output, relief='flat', bg='#44475a', fg='#f8f8f2', cursor="hand2", font='Helvetica 8 bold')
